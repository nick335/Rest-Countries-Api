...
import { handleResize, selectRegion, handleSearch } from "../store/features/country/countrySlice";
...
export default function Search(){
  ...
  const {width, region, darkTheme, searchInput } = useSelector(state => state.country)
  ...
  return(
    <section className="flex flex-col w-11/12 mx-auto lg:w-full lg:px-14 xl:px-20 lg:flex-row lg:mx-0 lg:justify-between lg:items-center lg:mt-8">
      <div className="flex bg-neutralText dark:bg-darkThemeHeader w-full mx-auto mt-6 mb-7 px-6 py-3 shadow rounded-md lg:mt-0 lg:mb-0 lg:w-2/6 lg:mx-0 lg:py-3.5 trans">
        ...
        value = { searchInput }
        onChange = {(event) => dispatch(handleSearch(event))}
        />
      </div>
      <div className="bg-neutralText dark:bg-darkThemeHeader w-3/5 lg:w-1/5 trans">
        <FormControl fullWidth  size={`${width.innerWidth > 1024 ? '' : 'small'}`}>
          <Select
            className="trans !text-lightThemeText border-none dark:!text-neutralText "
            displayEmpty 
            value={region}
            onChange={(event) => dispatch(selectRegion(event)) }
            input={<OutlinedInput />}
            sk = {{ border: 0, color:'red' }}
            MenuProps={MenuProps}
